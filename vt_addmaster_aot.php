...
if(isset($_POST['coll']))
{
  ...
  else
  {
    echo ' <script type="text/javascript">
      ...
      echo "<br><br><div class='table-responsive col-sm-offset-4 col-sm-4'><table border='4' ><tr><th><h3>AREA OF TRAINING</h3></th><th><h3>DELETE</h3></th></tr>";
      //echo '<div class="container-fluid">';
      echo'<form method="post" action="vt_addmaster_aot.php">';
      while($row = mysqli_fetch_array($q1)) 
      {
        ...
        <tr>
          ...
          <td> <input name = "<?php echo $row['aot']; ?>" value="<?php echo $row['aot']; ?>" type="text"  readonly  ></td>
          <!-- <td> <button id="edit_submit2" type="submit" name ="edit_aot"  onclick="return confirm('Are you sure you want to EDIT this VT_NO\'s?')" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-pencil"></span></button></td> -->
              <td> <button id="edit_submit2" type="submit" name ="del_aot" value="<?php echo $row['aot']; ?>" onclick="return confirm('Are you sure you want to DELETE <?php echo $row['aot']; ?>')" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-trash"></span></button></td>
            </tr>
          ...
          if(isset($_POST['del_aot']))
          {
            
            $aot = $_POST['del_aot'];
            
            $query11 = "delete from master_aot where aot='$aot'";
            if(mysqli_query($link,$query11))
            {
              ...
              echo'<script>window.open("vt_addmaster_aot.php","_self");</script>';
