...
if(isset($_POST['coll']))
{
  ...
  else
  {
    echo ' <script type="text/javascript">
          ...
          <?php 
          $query1 = "select * from master_session";
      $q1 = mysqli_query($link,$query1);
      echo "<div class='table-responsive col-sm-offset-4 col-sm-4'><br><br><table class='tb' border='4' ><tr><th><h3>FINANCIAL YEAR</h3></th><th><h3>DELETE</h3></th></tr>";
      //echo '<div class="container-fluid">';
      echo'<div>';
      echo'<form method="post" action="vt_addmaster_session.php">';
      while($row = mysqli_fetch_array($q1)) 
      {
        ...
        <tr>
          ...
          <td> <input name = "<?php echo $row['session']; ?>" value="<?php echo $row['session']; ?>" type="text"  readonly  ></td>
          <!-- <td> <button id="edit_submit2" type="submit" name ="edit_aot"  onclick="return confirm('Are you sure you want to EDIT this VT_NO\'s?')" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-pencil"></span></button></td> -->
              <td> <button id="edit_submit2" type="submit" name ="del_session" value="<?php echo $row['session']; ?>" onclick="return confirm('Are you sure you want to DELETE <?php echo $row['session']; ?>')" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-trash"></span></button></td>
            </tr>
     ...
     if(isset($_POST['del_session']))
          {
            ...
            $session = $_POST['del_session'];
            $check = "select last_count from master_session where session='$session'";
            $c1 = mysqli_query($link,$check);
            $flag=0;
            ...
            if($flag==0)
            {
            $query11 = "delete from master_session where session='$session'";
            if(mysqli_query($link,$query11))
            {
              ...
              echo'<script>window.open("vt_addmaster_session.php","_self");</script>';
            
      }
      else
      {
        echo ' <script type="text/javascript">
                  alert("A VT_No. is already been alloted to this Finacial Year so can\'t Delete!!!");
                </script>
                ';
      }
    }
